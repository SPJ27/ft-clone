  ...
  <div className="px-5 py-4 border-b border-[hsl(22,34%,43%)] last:border-b-0">
    <div className="flex items-center gap-2 mb-2">
      ...
      <div className="text-[12px] text-[rgb(215,181,147)]">
        <span className="text-[rgb(249,229,197)] font-medium">{user}</span>{" "}
        worked on {title}
      </div>
    ...
    {devlog.images?.length > 0 && (
      <div
        className={`grid gap-2 ${devlog.images.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}
      >
        {devlog.images.map((img, i) => (
          <div
            key={i}
            className="relative rounded-xl overflow-hidden bg-[hsl(22,34%,35%)] aspect-video"
          >
            <Image
              src={img}
...
const ProjectListItem = ({ project, active, onClick }) => (
  ...
  >
    <div className="flex items-center justify-between gap-2 mb-1">
      <span className="text-[14px] font-medium text-[rgb(249,229,197)] truncate">
        {project.title}
      </span>
    ...
    <div className="text-[12px] text-[rgb(215,181,147)] mb-2">
      {project.user}
    </div>
    <div className="flex gap-3 text-[11px] text-[rgb(215,181,147)]">
      <span className="flex items-center gap-1">
        <FaClock className="text-[10px]" /> {project.hours}h {project.minutes}m
      </span>
      <span className="flex items-center gap-1">
        <FaFileAlt className="text-[10px]" /> {project.devlogs.length} devlogs
      </span>
    </div>
  </button>
);
    ...
    const load = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/ship/shipwright`,
        );
    ...
    <div className="min-h-screen bg-[rgb(249,229,197)] p-4 sm:p-6">
      <div className="max-w-200 mx-auto">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[22px] font-medium text-[#7b4942] leading-none">
              Shipwright
            </h1>
            <p className="text-[13px] text-[hsl(22,34%,45%)] mt-0.5">
              {projects.length} ships awaiting review
            </p>
          </div>
        </div>
        <div className="grid grid-cols-1 gap-y-5 md:grid-cols-7 items-start">
          <div className="col-span-3 max-w-xs shrink-0 bg-[#7b4942] border-4 border-[hsl(22,34%,51%)] rounded-lg overflow-hidden">
            <div className="px-4 py-3 border-b border-[hsl(22,34%,45%)]">
              ...
              <div className="text-[12px] text-[rgb(215,181,147)] mt-0.5">
                ...
                <div className="bg-[#7b4942] border-4 border-[hsl(22,34%,51%)] rounded-lg overflow-hidden">
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div>
                        <h2 className="text-[22px] font-normal text-[rgb(249,229,197)] leading-tight">
                          {selected.title}
                        </h2>
                        <p className="text-[13px] text-[rgb(215,181,147)] mt-0.5">
                          by {selected.user}
                        </p>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {[
                        {
                          icon: <FaClock />,
                          label: `${selected.hours}h ${selected.minutes}m logged`,
                        },
                        {
                          icon: <FaFileAlt />,
                          label: `${selected.devlogs.length} devlogs`,
                        },
                        {
                        ...
                        <div
                          key={i}
                          className="flex items-center gap-1.5 bg-[rgb(100,58,53)] rounded-full px-3 py-1 text-[12px] text-[rgb(215,181,147)]"
                        >
                          {s.icon && (
                            <span className="text-[11px]">{s.icon}</span>
                          )}
                          <span className="font-medium text-[rgb(249,229,197)]">
                            {s.label}
                          </span>
                        </div>
                      ))}
                    ...
                    <p className="text-[13px] text-[rgb(215,181,147)] leading-relaxed mb-4">
                      {selected.desc}
                    </p>
