...
const statusConfig = {
  PENDING: {
    ...
    classes: "bg-yellow-500 text-white border border-yellow-500/40",
  },
  APPROVED: {
    label: "Approved",
    classes: "bg-green-500 text-white border border-green-500/40",
  },
  REJECTED: {
    label: "Rejected",
    classes: "bg-red-500 text-white border border-red-500/40",
  },
};
...
const ProjectBanner = ({
  ...
  return (
    <div
      ...
      className="text-white border-4 sm:border-10 max-w-230 mx-auto border-[hsl(22.59,34.14%,51.18%)] shadow-2xl rounded-2xl p-2 sm:p-3 px-3 sm:px-5 bg-[#7b4942]"
    >
      ...
      <div
        className={`grid px-2 sm:px-3 gap-3 mt-4  ${showShipButton ? "sm:grid-cols-3 grid-cols-1" : "sm:grid-cols-2 grid-cols-1"}`}
      >
        <Link
          href={demo}
          className="flex text-[rgb(245,216,198)] bg-[rgb(78,44,51)] justify-center items-center py-4 sm:py-5 px-8 text-lg sm:text-xl rounded-lg h-12 sm:h-10 gap-2 w-full"
        >
          <FaGlobe /> Demo
        </Link>
        <Link
        ...
        {showShipButton && (
          <Link
            href={`/projects/ship/${id}`}
            className={`flex ${canShip ? "text-[rgb(245,216,198)] bg-[rgb(78,44,51)]" : "pointer-events-none cursor-not-allowed text-[rgb(187,157,140)] bg-[rgb(133,94,101)]"} justify-center items-center py-4 sm:py-5 px-8 text-lg sm:text-xl rounded-lg h-12 sm:h-10 gap-2 w-full`}
          >
            <FaRocket /> Ship
          </Link>
        )}
